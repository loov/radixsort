...
func uint_32_large(arr, buf []uint) {
	prev := arr[0]
	sorted := true
...
// uint_64 implements radix sort using secondary buffer.
//
// len(buf) must equal len(arr)
...
func uint_64_large(arr, buf []uint) {
	prev := arr[0]
	sorted := true
